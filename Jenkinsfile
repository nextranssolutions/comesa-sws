  ...
  stages {
        stage('Check Modified Files') {
            steps {
                script {
                    ...
                    def angularPaths = ["developmentv2/web-application/public/views/dev_portal/comesa-sws/"]

                    // Backend files are all files that are NOT in angularPaths
                    def backendChanges = changes.findAll { file -> !angularPaths.any { file.startsWith(it) } }

                    // Determine build triggers
                    env.BUILD_BACKEND = backendChanges ? "true" : "false"
        ...
        stage('PRODUCTION OF BACKEND') {
                ...
                expression { return env.BUILD_BACKEND == "true" }
                    ...
                    steps {
                        sh '''#!/bin/bash
                        ###### PRODUCTION OF BACKEND ####################
                        cd $WORKSPACE/developmentv2/web-application
                ...
                expression { return env.BUILD_ANGULAR == "true" }
                    ...
                    steps {
                        ...
                        init_file="$WORKSPACE/developmentv2/web-application/public/init.blade.php"
                        ...
                        cp $WORKSPACE/developmentv2/web-application/public/views/front-end/*.js $WORKSPACE/developmentv2/web-application/public/

                        #Copy asset files
                        cp -R $WORKSPACE/developmentv2/web-application/public/views/front-end/assets $WORKSPACE/developmentv2/web-application/public/
