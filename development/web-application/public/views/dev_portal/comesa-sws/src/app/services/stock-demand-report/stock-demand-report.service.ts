...
export class StockDemandReportService {
  ...
  constructor(private HttpClient: HttpClient, private http: HttpClient,private sanitizer:DomSanitizer,private authService: AuthenticationService) { 
    this.baseUrl = AppSettings.base_url + '/api/stockdemandreport';
  }
  ...
  onLoadStockDemandData(data) {
    ...
    return this.HttpClient.get(this.baseUrl + '/onLoadStockDemandData', this.product)
      .pipe(map(data => {
            return <any>data;
      }));
  }
  onSaveStockDemandDetails(table_name,data,action_url){
  ...
  onDeleteStockDemandDetails(dataForm,table_name,title){
    ...
    return this.http.post(this.baseUrl + '/onDeleteStockDemandDetails', dataForm, this.product)
      .pipe(map(data => {
        return data;
      }));
  }
