...
export class AppsignUpComponent {
  ...
  onLoadsystemSignInUpGuidelines() {
    ...
    this.admnistrationService
      .onLoadDataUrl(data_submit, 'onLoadsystemSignInUpGuidelines')
      .subscribe(
        (data) => {
  ...
  onAccountTypeSelection($event) {
    if ($event.selectedItem) {
      ...
      this.has_partnerstate_defination =
        account_type.has_partnerstate_defination;
  ...
  filterCountries(accountType: any) {
    if (accountType.has_partnerstate_defination) {
      this.Countries = this.allCountries.filter(
        (country) => country.is_member_state
      );
    } else {
      this.Countries = this.allCountries;
    }
